...
export class PublicadorPanel implements vscode.WebviewViewProvider {
    ...
    constructor(
        ...
        private readonly context: vscode.ExtensionContext // <- importante
    ) {}
    ...
    resolveWebviewView(view: vscode.WebviewView) {
        ...
        view.webview.onDidReceiveMessage(async (dados) => {
            if (dados.acao === 'publicar') {
                ...
                // Salvar os dados no globalState
                this.context.globalState.update('publicadorIIS.dados', { server, sitePath, urlDestino, user, password });
                ...
                const pubxmlDir = path.join(projetoPath, 'Properties', 'PublishProfiles');
  ...
  <PropertyGroup>
    ...
    <UseMsdeployExe>True</UseMsdeployExe>
                ...
                vscode.window.withProgress({
                    ...
                    try {
                        ...
                        cp.exec(comando, (error, stdout, stderr) => {
                            if (error) {
                                vscode.window.showErrorMessage(`Erro na publicação: ${error.message}`);
                                return;
                            }

                            vscode.window.showInformationMessage('Publicação concluída com sucesso!', 'Abrir site', 'Usar outros dados')
                                .then(opcao => {
                                    if (opcao === 'Abrir site') {
                                        vscode.env.openExternal(vscode.Uri.parse(urlDestino || `https://${server}`));
            ...
            if (dados.acao === 'carregarDados') {
                ...
                view.webview.postMessage({ acao: 'preencherCampos', dados: dadosSalvos });
            }
        });
    }
...
<head>
    ...
    <style>
        ...
        input, button { width: 100%; padding: 6px; margin-top: 6px; }
...
<body>
    ...
    <form id="formulario">
        ...
        <input type="text" id="sitePath" required>

        <label>URL publicada:</label>
        <input type="text" id="urlDestino">
